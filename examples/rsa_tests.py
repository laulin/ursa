...
def test_encrypt_decrypt():
    ...
    (pub, priv) = ursa.newkeys(256, log_level="DEBUG")
    msg = "blinka".encode("utf-8")
    msg_enc = ursa.encrypt(msg, pub)
    msg_dec = ursa.decrypt(msg_enc, priv)
...
def test_mod_msg():
    ...
    except ursa.pkcs1.DecryptionError:
        pass
...
def test_randomness():
    ...
    (pub, priv) = ursa.newkeys(256, log_level="DEBUG")
    msg = "blinka".encode("utf-8")
    msg_enc_1 = ursa.encrypt(msg, pub)
    msg_enc_2 = ursa.encrypt(msg, pub)
...
def test_sign_verify_sha256():
    ...
    (pub, priv) = ursa.newkeys(496, log_level="DEBUG")
    msg = "red apple"
    signature = ursa.sign(msg, priv, "SHA-256")
    ursa.verify(msg, signature, pub)


def test_sign_verify_sha384():
    """Test SHA-384 sign and verify the message."""
    (pub, priv) = ursa.newkeys(624, log_level="DEBUG")
    msg = "red apple"
    signature = ursa.sign(msg, priv, "SHA-384")
    ursa.verify(msg, signature, pub)


def test_sign_verify_sha512():
    """Test SHA-512 sign and verify the message."""
    (pub, priv) = ursa.newkeys(752, log_level="DEBUG")
    msg = "red apple"
    signature = ursa.sign(msg, priv, "SHA-512")
    ursa.verify(msg, signature, pub)


def test_sign_verify_fail():
    """Check for ursa.pkcs1.VerificationError on
    ...
    try:
        ursa.verify(msg, signature, pub)
    except ursa.pkcs1.VerificationError:
        # Expected error
        pass
